...
void printLevelTree(BinTree p, int h, int d)
{
    int space;
    if (!isTreeEmpty(p))
    {
        ...
        printf("%d\n", ROOT(p));
        printLevelTree(LEFT(p), h, d + 1);
        printLevelTree(RIGHT(p), h, d + 1);
    }
}

void printTree(BinTree p, int h)
{
    printLevelTree(p, h, 0);
}
...
int rob1(BinTree root)
{
    ...
    int left = rob1(LEFT(root));
    int right = rob1(RIGHT(root));

    return left > right ? ROOT(root) + left : ROOT(root) + right;
}
...
int rob2(BinTree root)
{
    ...
    {
        sum01 += rob2(LEFT(LEFT(root)));
        sum01 += rob2(RIGHT(LEFT(root)));
    }
    if (!isTreeEmpty(RIGHT(root)))
    {
        sum01 += rob2(LEFT(RIGHT(root)));
        sum01 += rob2(RIGHT(RIGHT(root)));
    }

    sum02 += rob2(LEFT(root));
    sum02 += rob2(RIGHT(root));

    return sum01 > sum02 ? sum01 : sum02;
}
